...
EnemyDarkSkeleton::EnemyDarkSkeleton() : EnemyCharacter(L"Enemy::EnemyDarkSkeleton")
{
}
...
void EnemyDarkSkeleton::OnAutoAttackShot(ActorPtr inVictim)
{
...
void EnemyDarkSkeleton::OnAutoAttackTargeting(const float inDamage, const FVector inRange)
{
	GameWorldPtr world = std::static_pointer_cast<GameWorld>(GetWorld().lock());
	if (nullptr == world)
	{
...
void EnemyDarkSkeleton::OnStunWakeUp()
{
	if (false == this->IsDeath())
	{
		if (this->mStateManager.GetCurrentStateType() == EStateType::State_Stun)
...
void EnemyDarkSkeleton::OnReward()
{
	WorldPtr world = this->GetWorld().lock();
	if (nullptr == world)
	{
